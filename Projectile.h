...
public:
	...
	Projectile(Vector2D p_position, Vector2D destination) : Entity(){
		...
		colliders.colliders.push_back(Collider(position.x + SCALE, position.y, 2 * SCALE, 4 * SCALE));
		colliders.colliders.push_back(Collider(position.x, position.y + SCALE, 4 * SCALE, 2 * SCALE));
